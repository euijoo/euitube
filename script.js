...
const INITIAL_MAIN_COVERS = [
  ...
  "https://i.pinimg.com/1200x2047/fc/20/47/fc20...ae76fc8.jpg",
...
const firebaseConfig = {
  apiKey: "AIzaSyA-wThpRQqn1XaB8sIBO4J4MqkOQyTy04",
...
async function fetchVideoInfo(videoId) {
  ...
  if (!res.ok) throw new Error("YouTube Data API error.");
...
async function fetchPlaylistItems(playlistId, maxTotal = 50) {
  ...
  while (videoIds.length < maxTotal) {
    ...
    const res = await fetch(`${endpoint}?${params.toString()}`);
    if (!res.ok) throw new Error("YouTube Playlist API error.");
    const data = await res.json();
...
async function deleteAlbum(album) {
  try {
    ...
    tracks.forEach(t => {
      if (t.albumId === album.id) t.albumId = null;
    });
...
function createTrackListItem(track) {
  ...
  channelDiv.className = "track-item-channel";
  channelDiv.textContent = track.channel;

  textBox.appendChild(titleDiv);
  textBox.appendChild(channelDiv);
  ...
  li.addEventListener("click", e => {
    ...
    document
      .querySelectorAll(".track-item.active")
...
function createAlbumItem(album, albumTracks) {
  ...
  // ✅ 앨범 커버는 album.coverUrl만 사용
  const thumbSrc = album.coverUrl || null;
  if (thumbSrc) {
    thumb.src = thumbSrc;
  } else {
  ...
  albumMenuBtn.addEventListener("click", e => {
    ...
    if (!isOpen) {
      ...
      albumMenu.style.visibility = "visible";
    }
  });
  ...
  changeCoverAlbumItem.addEventListener("click", e => {
    ...
    const currentCover = album.coverUrl || null;
    ...
    const handleSave = async () => {
      ...
      try {
        ...
        // ✅ 현재 선택된 앨범이면 메인 썸네일 갱신
        if (currentAlbumId === album.id) {
          if (newCover) thumbnailEl.src = newCover;
          else thumbnailEl.removeAttribute("src");
        }

        // ✅ 리스트의 앨범 썸네일도 album.coverUrl로만 갱신
        if (newCover) thumb.src = newCover;
        else thumb.removeAttribute("src");

        renderTrackList();
      } catch (err) {
        alert("Album cover update failed.");
      } finally {
        hideCoverSheet();
  ...
  renameItem.addEventListener("click", async e => {
    e.stopPropagation();
    albumMenu.classList.remove("open", "open-up");
    const input = prompt("Rename album.", album.name);
...
function renderTrackList() {
  ...
  mainTracks.forEach(track => {
    mainUl.appendChild(createTrackListItem(track));
  });
...
function ensureAlbumSheet() {
  if (albumSheetBackdrop) return albumSheetBackdrop;
  ...
  newInput.type = "text";
  newInput.placeholder = "New album name";
...
function showAlbumSelectSheet(track) {
  ...
  currentEl.textContent = `Current: ${currentAlbumName}`;
  listBox.innerHTML = "";
  ...
  const handleCreate = async () => {
    ...
    if (newAlbum && currentTrackId === track.id) {
...
function showDeleteConfirm(onYes) {
  let backdrop = document.querySelector(".delete-confirm-backdrop.global");
  if (!backdrop) {
    backdrop = document.createElement("div");
    backdrop.className = "delete-confirm-backdrop global";
...
function closeAllTrackMenus() {
  document
    .querySelectorAll(".track-menu.open")
    .forEach(m => m.classList.remove("open", "open-up"));
  document
    .querySelectorAll(".album-menu.open")
    .forEach(m => m.classList.remove("open", "open-up"));
...
function resetNowPlayingUI() {
  ...
  if (miniTitleNew) miniTitleNew.textContent = "";
  if (miniArtistNew) miniArtistNew.textContent = "";
...
function updateNowPlaying(track) {
  ...
  if (coverUrl) thumbnailEl.src = coverUrl;
  else thumbnailEl.removeAttribute("src");
  ...
  if (miniThumbNew && miniTitleNew && miniArtistNew) {
    if (coverUrl) miniThumbNew.src = coverUrl;
    else miniThumbNew.removeAttribute("src");
...
function updateNewMiniPlayer() {
  ...
  const isPreview = !currentTrack || !playingId || playingId !== currentTrack.videoId;
  if (isPreview) {
    if (currentEl) currentEl.textContent = "00:00";
    if (totalEl) totalEl.textContent = "00:00";
    if (fillEl) fillEl.style.width = "0";
...
function updatePlayingIndicator() {
  document
    .querySelectorAll(".track-item.is-playing")
...
async function addFromInputUrl(url) {
  ...
  return await addTrackFromUrl(url);
}
...
googleLoginButton.addEventListener("click", async () => {
  try {
    ...
    if (
      ...
      loginError.textContent = "팝업이 차단되었거나 닫혔습니다.";
    } else {
      loginError.textContent = err.code || "로그인 실패";
    }
  }
});
...
if (logoutButton) logoutButton.addEventListener("click", confirmAndLogout);
if (userAvatarEl) {
...
if (appLogoButton) {
  appLogoButton.addEventListener("click", () => window.location.reload());
}
...
onAuthStateChanged(auth, async user => {
  ...
  if (user) {
    ...
    if (userAvatarEl) {
      if (freshUser.photoURL) userAvatarEl.src = freshUser.photoURL;
...
function ensureCoverSheet() {
  if (coverSheetBackdrop) return coverSheetBackdrop;
  ...
  title.textContent = "커버 이미지 URL";

  const desc = document.createElement("p");
  desc.className = "cover-sheet-desc";
  desc.textContent = "URL을 입력하고 Save를 눌러주세요.";
...
function openTitleRenameSheet(track) {
  ...
  if (titleElSheet) titleElSheet.textContent = "곡 제목 변경";
  if (descElSheet) descElSheet.textContent = "새 제목을 입력하세요.";
...
// ---------------------- 메인 Change Cover 버튼 (앨범/곡 분리) ----------------------
if (changeCoverBtn) {
  changeCoverBtn.addEventListener("click", () => {
    // 1) 앨범이 선택된 상태면 앨범 커버만 수정
    if (currentAlbumId) {
    ...
    // 2) 앨범이 아니라 곡이 선택된 상태면 곡 커버만 수정
    if (!currentTrackId) {
      alert("곡이나 앨범을 먼저 선택하세요.");
      return;
    }
    const track = tracks.find(t => t.id === currentTrackId);
    if (!track) return;
